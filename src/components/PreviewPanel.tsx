            ...
            <div className={`relative overflow-hidden ${aspectClasses[aspectRatio]}`}>
                ...
                {isLoading && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-black/25">
                        ...
                        <div className="text-center">
                            <p className="text-sm font-medium text-zinc-400">AI is processing your request...</p>
                            <p className="mt-1 text-xs text-zinc-400">This may take 30 seconds.</p>
                        </div>
                    </div>
                )}
                ...
                {!isLoading && !thumbnail?.image_url && (
                    <div className="absolute inset-0 m-2 flex flex-col items-center justify-center gap-4 rounded-lg border-2 border-dashed border-white/20 bg-black/25">
                        <div className="hidden sm:flex h-20 w-20 items-center justify-center rounded-full bg-white/10">
                            <ImageIcon className="size-10 text-white opacity-50" />
                        </div>

                        <div className="px-4 text-center">
                            <p className="text-zinc-200">Generate your first thumbnail.</p>
                            <p className="mt-1 text-xs text-zinc-400">Tell us your idea and click generate.</p>
                        </div>
                    </div>
                )}
            </div>
