		...
		<>
            ...
            <div className="mt-36 min-h-screen px-6 md:px-16 lg:px-24 xl:px-32">
                ...
                <div className="mb-8">
                    ...
                    <p className="text-sm text-zinc-400 mt-1">Manage and organize your AI-generated thumbnails</p>
                </div>
                    ...
                    <div className="columns-1 sm:columns-2 lg:columns-3 2xl:columns-4 gap-8">
                        {thumbnails.map((thumb: IThumbnail) => {
                            ...
                            return (
                                <div
                                    key={thumb._id}
                                    onClick={() => navigate(`/create/${thumb._id}`)}
                                    className="mb-8 group relative cursor-pointer rounded-2xl border border-white/10 transition shadow-xl break-inside-avoid"
                                >
                                    {/* IMAGE */}
                                    <div className={`relative overflow-hidden rounded-t-2xl ${aspectClass} bg-black`}>
                                        {thumb.image_url ? (
                                            ...
                                            <div className="w-full h-full flex items-center justify-center text-sm text-zinc-400">
                                                {thumb.isGenerating ? 'Generating...' : 'No preview available'}
                                            </div>
                                        )}

                                        {thumb.isGenerating && (
                                            <div className="absolute inset-0 bg-black/50 flex items-center justify-center text-sm font-medium">Generating...</div>
                                        )}
                                    </div>
                                    ...
                                    <div onClick={(e)=> e.stopPropagation()} className="absolute bottom-2 right-2 max-sm:hidden hidden group-hover:flex gap-1.5">
                                        ...
                                        <ArrowUpRightIcon
                                        onClick={() => window.open(`/preview?thumbnal_url=${encodeURIComponent(thumb.image_url || '')}&title=${encodeURIComponent(thumb.title || '')}`, "_blank")}
                                        className="size-6 bg-black/50 p-1 rounded hover:bg-pink-600 transition-all"/> 
                                    </div>
